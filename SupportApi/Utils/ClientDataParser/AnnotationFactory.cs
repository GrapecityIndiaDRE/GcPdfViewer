...
namespace SupportApi.Utils
{
    ...
    public class AnnotationFactory
    {
        ...
        /// <summary>
        /// Convert specified annotations to page content and remove them from the page.
        /// </summary>
        ...
        public static void UpdateAnnotationBase(AnnotationBase annotation, DataParser dataParser)
        {
            // DOC-1841:
            ...
            // DOC-3619:
            annotation.Name = dataParser.annotationName;
            if (dataParser.ContainsDataKey("contents"))
                annotation.Contents = dataParser.contents;
            if (dataParser.ContainsDataKey("locked"))
                annotation.Locked = dataParser.ParseBool("locked");
        }

        private static void _UpdateMarkupAnnotation(MarkupAnnotation annotation, DataParser dataParser)
        {
            UpdateAnnotationBase(annotation, dataParser);
            ...
            if (!string.IsNullOrEmpty(dataParser.fileId))
            {
                ...
                if (stream != null)
                {
                    try
                    {
                        ...
                        fxo.Graphics.DrawImage(image, fxo.Bounds, null, GrapeCity.Documents.Drawing.ImageAlign.StretchImage, annotation.Opacity);
        ...
        private static AnnotationBase UpdateAnnotation(LinkAnnotation annotation, DataParser dataParser)
        {
            ...
            switch (linkType)
            {
                ...
                default:
#if DEBUG
                    System.Diagnostics.Debug.Assert(false, $"Unknown linkType: {linkType}");
#endif
                    break;
            }
        ...
        {
            annotation.TextOffsets = /* DOC-1990: GcPdfViewer always uses Rect as Text rectangle.*/ null; 
            _UpdateMarkupAnnotation(annotation, dataParser);
            if (dataParser.borderWidth.HasValue)
                annotation.LineWidth = dataParser.borderWidth.Value;
