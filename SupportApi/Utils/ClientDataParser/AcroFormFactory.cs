...
namespace SupportApi.Utils
{
    public class AcroFormFactory 
    {
        ...
        public static void UpdateFieldWidget(Field field, WidgetAnnotation widget, DataParser dataParser)
        {
            ...
            AnnotationFactory.UpdateAnnotationBase(widget, dataParser);
            ...
            {
                // Acrobat Reader DC (2020.006.20042) splits nested field names using dot, so, we do same for child field:
        ...
        {
            bool SubmitForm = dataParser.ParseBool("SubmitForm") || dataParser.ParseBool("submitForm");
            bool ResetForm = dataParser.ParseBool("ResetForm") || dataParser.ParseBool("resetForm");
            UpdateFieldWidget(field, field.Widget, dataParser);
            if (ResetForm)
            {
